...
if __name__ == "__main__":
    ...
    # Create cleaner and run it
    cleaner = OutputCleanerAgent()
    result = cleaner.clean_output_directory(dry_run=False)

    # Print summary
    if result["status"] == "success":
        print(f"Successfully deleted {result['statistics']['files_deleted']} files ({result['statistics']['total_size_deleted_mb']:.2f} MB)")
    else:
        print("Cleaning failed:", result.get("message", "Unknown error"))